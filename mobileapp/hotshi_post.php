...
{ 
    ...
    return 0; 
        ...
        $offset = $_POST['OFFSET']; 
        $id_user = $_POST['id']; 
		$stmt = $conn->prepare("SELECT fullname,users.id as author_id,users.email as auth_email,description,posts.id,posts.date_created as post_date,users.image as post_owner_pic,posts.image as post_image,users.profession FROM `posts` INNER JOIN `users` ON users.id=posts.user_id WHERE posts.is_deleted=0 ORDER BY posts.id DESC LIMIT 20 OFFSET $offset");       
		 //  LIMIT 20 OFFSET $offset
