    ...
    if( $('#customCheckoutButton').length ) {
        ...
        var handler = StripeCheckout.configure({
            ...
            token: function(token) {
                ...
                $.ajax({
                    type : 'POST',
                    url  : '/ajax/account/create_project',
                    data : message,
                    dataType    : "json",
                    success: function(data){
                        ...
                        if( data.status == 'error' ) {
                            ...
                            $('#create-project-form').append('<input type="hidden" name="project_id" value="'+data.project_id+'">');
                            var $formData = new FormData($('#create-project-form')[0]);
                            $.ajax({
                                type : 'POST',
                                url  : '/ajax/account/upload_project_image',
                                data : $formData,
                                dataType    : "json",
                                processData: false,
                                contentType: false,
        ...
        document.getElementById('customCheckoutButton').addEventListener('click', function(e) {
            ...
            if( $('#title').val() && $('#description').val() && $('#tags_1').val() && $('#qualifications').val() && $('#location').val() ) {
                ...
                handler.open({
                    ...
                    zipCode: false,
                    currency : 'eur',
                    amount: (parseFloat($('#js_create_project_cost').val()))*100 //in cents
                });
            }
            //e.preventDefault();
        });
